...
class WebOMatic_Precision:
    ...
    def find_best_match_in_zone(self, image_path, ref_image_path, grid_zone, confidence_threshold=0.75):
        ...
        try:
            ...
            # Calculate coordinates in the scaled screenshot space
            ...
            print(f"✅ Found match at scaled coordinates ({int(absolute_x)}, {int(absolute_y)}) with confidence {max_val:.3f}")
            return match_result
    ...
    def precision_click(self, target_name, screenshot_path=None):
        ...
        center_x = int(match["center_x"] / self.scale_factor)
        center_y = int(match["center_y"] / self.scale_factor)
        confidence = match["confidence"]

        print(f"🎯 Target found at ({center_x}, {center_y}) with {confidence:.3f} confidence (scaled down by {self.scale_factor:.2f})")
