...
def grid_to_coords(cell_id):
    ...
    left = 1023 + col * 85
    top = 26 + row * 130
    return (left, top, 85, 130)

def match_reference_in_cell(screenshot_path, ref_name, cell_id):
    ref_path = os.path.expanduser(f"~/Desktop/web_o_matic/kai_ui_refs/{ref_name}.png")
    image = cv2.imread(screenshot_path)
    ref = cv2.imread(ref_path)
    if image is None or ref is None:
        print(f"❌ Could not load screenshot or reference image.")
        return None
    ...
    if max_val >= threshold:
        ...
        print(f"🎯 Ref match at ({center_x}, {center_y}) with confidence {max_val:.2f}")
        return {
        ...
        print(f"⚠️ No confident match in {cell_id}. Max score: {max_val:.2f}")
        return None
...
class KaiNavigationSystem:
    ...
    def parse_kai_response_to_json(self, response_text):
        ...
        # FIXED: Corrected regex patterns for reference matching
        grid_match = re.search(r'grid_cell\s*[:=]\s*["\']([A-L][1-8])["\']', response_text)
        target_match = re.search(r'target_hint\s*[:=]\s*["\'](\w+)["\']', response_text)

        if grid_match and target_match:
            cell_id = grid_match.group(1)
            target_name = target_match.group(1)
            print(f"🔍 Trying reference match: {target_name} in {cell_id}")
            match = match_reference_in_cell(self.screenshot_path, target_name, cell_id)
            if match:
                intent_data = {
                    "intent": f"Ref-matched click on {target_name}",
                    "grid_cell": cell_id,
                    "coordinates": {
                        "top": match["top"],
                        "left": match["left"],
                ...
                with open(self.intent_path, 'w') as f:
                    json.dump(intent_data, f, indent=2)
                print(f"✅ Ref-match JSON created")
                return True
                ...
                try:
                    ...
                    if timestamp != last_timestamp:
                        ...
                        if safety == "green" and not confirm:
                            ...
                            # 🌀 Glide to target (simulate human-like mouse motion)
                            start_x, start_y = 1800, 900  # Starting from lower-right
                            end_x, end_y = x, y          # Target coordinates from JSON
                            
                            # Glide in 15 steps to trigger hover states
                            for i in range(15):
                                step_x = int(start_x + (end_x - start_x) * i / 14)
                                step_y = int(start_y + (end_y - start_y) * i / 14)
                                pyautogui.moveTo(step_x, step_y, duration=0.01)
                            
                            # Click at destination
                            time.sleep(0.2)
                            pyautogui.click(button='left', clicks=1, interval=0.25)
                            print(f"🧭 Glide-clicked target at ({x}, {y})")
                        else:
